...
class kline(object):  # 返回
    ...
    @staticmethod
    def get_trading_days(start: Optional[str] = None, end: Optional[str] = None, target: str = 'us_stock') -> Optional[
        List[pd.Timestamp]]:
        # 根据target类型选择因子文件路径
        if target == 'us_stock':
            factor_file = r'D:\Python\HKUST_IP2\factor\美股因子.csv'
        else:
            factor_file = r'D:\Python\HKUST_IP2\factor\台股因子.csv'
    ...
    def load_data(self, dates: List[pd.Timestamp], codes: Optional[List[str]] = None) -> pd.DataFrame:
        ...
        if self.target == 'us_stock':
            # 美股数据路径
            data_path = r'D:\Python\HKUST_IP2\sp500_processedOHLC_CRSP'
        elif self.target == 'tw_stock':
            # 台股数据路径
            data_path = r'D:\Python\HKUST_IP2\tw50_processedOHLC'
...
class Factor_Analysis(object):  # change your save_path
    ...
    def __init__(self, factor_df: pd.DataFrame, factor_name: Optional[str] = None, codes: Optional[List[str]] = None,
        ...
        freq_dict = {"D": "daily", "W": "weekly", "M": "monthly"}
        self.freq_used = freq_dict[freq]
        self.codes = codes
        self.BENCHMARK_PATH = "D:\Python\HKUST_IP2\merged_factors\SPX.xlsx" if target == 'us_stock' else "D:\Python\HKUST_IP2\merged_factors\TWII.xlsx"
    ...
    def preprocess(self) -> pd.DataFrame:  # 因子截面标准化+去极值
        standard_factor = self.factor_df.apply(lambda x: self.standard(cs_factor=x, type='Normal'), axis=1).dropna(
            axis=1, how='all')
        ...
        if len(dup_pos_list) >= 3:
            ...
            if dup_pos_list[-1] == len(factor_quantiles) - 1:
                ...
                while len(dup_pos_list) != 0:
                    ...
                    factor_quantiles.iloc[dup_pos] = (factor_quantiles.iloc[dup_pos] + factor_quantiles.iloc[
                        dup_pos + 1]) / 2
                    dup_pos_list = np.where(factor_quantiles.duplicated())[0]
    ...
    def hit_rate(self) -> pd.DataFrame:
        ...
        for i in range(len(date_range)):
            ...
            ## !!!!!!!!!!!!!!!!!!!!!!!!!debug 要是没有日期就返回nan!!!!!!!!!!!!!!!!!!!!!!
            benchmark_return = self.benchmark_r_next.get(i, np.nan)
    ...
    def Backtest_Plot(self):
        ...
        save_dir = os.path.join(
            rf'D:\Python\HKUST_IP2\factor_multi_freq\factor_result_{self.freq_used}\backtest_result\因子分层表现',
            f'{self.target}', f'{self.name}')
        if not os.path.exists(save_dir):
            make_dir(save_dir)
        plt.savefig(
            os.path.join(save_dir, f'start={dt.strftime(self.trading_day[0], "%Y%m%d")} freq={self.freq} 分层回测.png'))
        plt.close()
    ...
    def bar_plot(self, figure_name: str = 'turnover'):
        ...
        save_dir = os.path.join(
            rf'D:\Python\HKUST_IP2\factor_multi_freq\factor_result_{self.freq_used}\backtest_result\因子回测结果',
            f'{self.target}',
            f'{self.name}')
        if not os.path.exists(save_dir):
            make_dir(save_dir)
    ...
    def factor_return(self) -> pd.Series:
        ...
        for date in self.trading_day:
            ...
            ##############################################debug！！！！！！！############################################
            # 计算因子收益率
            if daily_factor.empty or daily_next_r.empty:
    ...
    def comprehensive_analysis_plot(self):
        ...
        save_path = os.path.join(save_dir,
                                 f'{self.name}_综合分析_开始于{dt.strftime(self.trading_day[0], "%Y%m%d")}_frequency={self.freq}.png')
...
def single_factor_test(factor_df, col, freq='D', target='us_stock', start='20200101',
                       end='20250101'):  # freq设置D,W,M,Y，即回测频率
    # try:
    print(col)
...
if __name__ == '__main__':  # 因子值最好是pivot那种类型的，不是的话也行
    base_factor_path = r"D:\大学文档\MAFM\IP\AtrDataCode\backtest_result\sp500_factor_result"
    target = 'us_stock'
    folder_name = 'tw50'  # 数据目录名称（例如 tw50）
    # 获取需要计算的因子
    base_output_dir = rf"D:\Python\HKUST_IP2\factor_multi_freq\factor_result_weekly\backtest_result\因子回测结果\tw_stock"

    stock_path = fr"{base_factor_path}\{folder_name}"
    ...
    print(f"要排除的标的: {len(remove_tickers)}")
    #
    # # 严格完全匹配（不含扩展名）
    # alpha_list = [p for p in alpha_list if os.path.splitext(os.path.basename(p))[0] not in remove_tickers]
    #
    # print(f"筛选后剩余 {len(alpha_list)} 个因子文件")
    #
    ...
    # tasks = [(alpha_df_list[i], col_list[i], 'W', 'tw_stock', '20030101', '20250101') for i in
    #          range(len(alpha_df_list))]
    # print("weekly")
